...
function usernameSelect(){
    ...
    $(function () {
        $.ajax({
            type: "POST",
            url: "/usernameinformation/querySchoolName",
            dataType: "json",
            cache: false,
            success: function (data) {
                $('#username').append(new Option());
                $.each(data, function (index, item) {
                        $('#username').append(new Option(item.siName, item.id));
...
function siIdSelect() {
    layui.use(['form','jquery'],function () {
        ...
        form.on('select(Type_filter)', function(o){
            id = o.value;
            $.get('/usernameinformation/getText/'+id,function (r) {
