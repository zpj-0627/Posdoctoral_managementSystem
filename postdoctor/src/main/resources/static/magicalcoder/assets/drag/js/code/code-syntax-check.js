...
CodeSyntaxCheck.prototype.syntax = function (node) {
    ...
    switch (identifier) {
        ...
        case 'mc-href-url':
            this.hrefUrlSyntax(node);
            break;
        case 'mc-href-param-name':
            this.hrefParamNameSyntax(node);
            break;
        case 'mc-href-param-value':
            this.hrefParamValueSyntax(node);
            break;
...
CodeSyntaxCheck.prototype.mcSetValueSyntax = function(node){
    ...
    if(node.magicalCoder.children.length<=0){
        ...
        if(this.lowCodeUtil.nodeUtil.pubIsOnlyOneTextNodeChildren(node)){
            var content = this.lowCodeUtil.nodeUtil.pubGetNodeContent(node);
            if(content===''){
                this.setErrorInfo(node,['【值】请填写'],function (nodeItem,idx) {
...
CodeSyntaxCheck.prototype.hrefUrlSyntax = function(node){
    var _t = this;
    if(this.lowCodeUtil.stringUtil.isBlank(node.attributes['value'])){
        this.setErrorInfo(node,['地址不能为空'],function (nodeItem,idx) {
            _t.api.focus([node.magicalCoder.id]);
        },true);
        return;
    }
...
CodeSyntaxCheck.prototype.hrefParamNameSyntax = function(node){
    var _t = this;
    if(this.lowCodeUtil.stringUtil.isBlank(node.attributes['value'])){
        this.setErrorInfo(node,['请配置参数名'],function (nodeItem,idx) {
            _t.api.focus([node.magicalCoder.id]);
        },true);
        return;
    }
...
CodeSyntaxCheck.prototype.hrefParamValueSyntax = function(node){
    var _t = this;
    if(this.lowCodeUtil.stringUtil.isBlank(node.attributes['value'])){
        this.setErrorInfo(node,['请配置参数值'],function (nodeItem,idx) {
            _t.api.focus([node.magicalCoder.id]);
        },true);
        return;
    }
...
CodeSyntaxCheck.prototype.mcSetVariableSyntax = function(node){
    ...
    if(node.magicalCoder.children.length<=0){
        ...
        if(this.lowCodeUtil.nodeUtil.pubIsOnlyOneTextNodeChildren(node)){
            var name = this.lowCodeUtil.nodeUtil.pubGetNodeContent(node);
            if(name===''){
                this.setErrorInfo(node,['【赋值的变量名】不能为空'],function (nodeItem,idx) {
                    layer.msg("取个规范的英文字符的名称吧");
                    _t.api.focus([node.magicalCoder.id]);
                });
                return;
            ...
            /*if(!(/^[a-zA-Z][a-zA-Z0-9_.]*$/g.test(name))){
                this.setErrorInfo(node,['【不能为空】取名规范：请使用字母数字或下划线组成。必须以字母或下划线开头'],function (nodeItem,idx) {
                    layer.msg("取个规范的英文字符的名称吧");
                    _t.api.focus([node.magicalCoder.id]);
                });
                return;
