...
_CodeTranslateUtil.prototype.selectValue = function(node){
...
_VueCodeTranslate.prototype.startTag = function(node,jsArr){
    ...
    switch(identifier){
        ...
        case 'mc-ajax-method':
            jsArr.push("method:\"");
            var method = this.codeTranslateUtil.selectValue(node);
            if(method===''){
                method = 'post';
            }
            jsArr.push(method);
        ...
        case 'mc-ajax-params':
            ...
            if(methodNode){
                var method = this.codeTranslateUtil.selectValue(methodNode);
                if(method == 'get'){
                    jsArr.push("params:{");
                }else {
                    jsArr.push("data:{");
        ...
        default:
            var parentNode = this.nodeMap[node.magicalCoder.parentId];
            if(parentNode.magicalCoder.identifier=='option'){
                break;
            }
