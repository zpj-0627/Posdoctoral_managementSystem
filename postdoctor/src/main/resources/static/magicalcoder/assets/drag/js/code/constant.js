...
function Constant(){
    ...
    this.settings.other.top.mode.resize=false;
    this.settings.other.top.review.resize=false;
    ...
    this.components = [
        {
            ...
            children:[
                    ...
                    html:
                        '<div class="layui-row mc-function">'+
                            '<div class="layui-col-xs12 mc-function-head">'+
                                '<input class="layui-input mc-function-name" type="text" placeholder="方法名"/>'+
                                '<div class="layui-col-xs12 mc-function-params">'+
                                    '<input class="layui-input mc-function-param" type="text" placeholder="参数"/>'+
                                    '<input class="layui-input mc-function-param" type="text" placeholder="参数"/>'+
                                '</div>'+
                            '</div>'+
                ...
                },
                {
                    name:"接口调用",
                    icon:"assets/drag/img/left/around1.png",
                    html:"<div class='layui-row mc-ajax'><select lay-filter='mc-ajax-method' class='mc-ajax-method'><option value='post'>POST</option><option value='get' selected>GET</option></select><input class='layui-input mc-ajax-url' type='text' placeholder='请求地址'/><div class='layui-col-xs12 mc-ajax-params'><div class='layui-col-xs12 mc-ajax-param'><input class='layui-input mc-ajax-param-name' type='text' placeholder='参数名'/><input class='layui-input mc-ajax-param-value' type='text' placeholder='参数值'/></div></div><div class='layui-col-xs12 mc-ajax-thens'></div></div>",
                },
                {
                    name:"页面跳转",
                    icon:"assets/drag/img/left/around1.png",
                    html:"<div class='layui-row mc-href'><input class='layui-input mc-href-url' type='text'  placeholder='跳转地址'/><div class='layui-col-xs12 mc-href-params'><div class='layui-col-xs12 mc-href-param'><input class='layui-input mc-href-param-name' type='text'  placeholder='参数名'/><input class='layui-input mc-href-param-value' type='text' mc-is-variable  placeholder='参数值'/></div></div></div>",
                },
                {
                    name:"页面刷新",
                    icon:"assets/drag/img/left/around1.png",
    ...
    * tmpWrapTag:'div'临时包裹标签 有些结构被layui写的是平级 放在了元素标签下面 这样拖拽插件就察觉不到 此时需要我们包裹一个临时标签 比如select layui-table这些*/
    ...
    var executeBlockParents=['mc-root','mc-if-execute','mc-ajax-thens','mc-function-body'];//想执行的都放这里面
    ...
    this.tagClassMapping = {
        "mc-root":{name:"根面板",dragInto:true,duplicate:false, copy:false,      paste:true,    canDelete:false,onlyChildren:['mc-function']},
        ...
        "mc-set":    {name:"赋值语句",dragInto:true,duplicate:true,duplicateAttr:[],        copy:true,       paste:false, canDelete:true,assistDelete:true,onlyParents:executeBlockParents, onlyChildren:['mc-defined-variable','mc-set-to'].push(supportQianTao)    },
            "mc-defined-variable":{name:"自定义的变量名",treeExtraName:{attr:[],text:true},canNotOperate:true,dragInto:true, copy:false,      paste:false,    canDelete:false,childrenLimit:0,dblClickChange:{type:"text"}},
            "mc-set-variable":{name:"设置的变量名",treeExtraName:{attr:[],text:true},canNotOperate:true,dragInto:true, copy:false,      paste:false,    canDelete:false,childrenLimit:0,dblClickChange:{type:'text'}},
            "mc-set-to":{name:"赋值为",treeExtraName:{attr:[],text:true},dragInto:false, canNotOperate:true,copy:false,      paste:false,    canDelete:false},
            "mc-set-value":{name:"值",canZoom:true,assistZoom:true,dragInto:true,canNotOperate:true,duplicate:false, copy:false,      paste:true,    canDelete:false,childrenLimit:1,dblClickChange:{type:'text'}},
        ...
        "mc-function":    {name:"方法函数",canZoom:true,assistZoom:true,dragInto:true,duplicate:true,duplicateAttr:[],        copy:true,       paste:false, canDelete:true,assistDelete:true,onlyParents:['mc-root'], onlyChildren:['mc-function-head','mc-function-body']},
        ...
        [
            {
                ...
                content:{
                    ...
                    "mc-ajax-param-value":[
                        {identifier:"variable",type:this.type.SELECT       ,clearAttr:true   ,oneLine:false        ,change:this.change.ATTR    ,title:'参数值' ,attrName:'value' ,options:[],                                                            },
                    ...
                    "mc-set-variable":[
                        {identifier:"variable",type:this.type.SELECT       ,clearAttr:true   ,oneLine:false        ,change:this.change.TEXT    ,title:'可用变量'  ,options:[],                                                            },
                    ],
                     "mc-function-name":[
