...
MagicalCallback.prototype.pre_build_attrs = function (param) {
	...
	//示例一：省市区级联功能 加在ecahrts线性表组件上吧
	{
		if(focusNode.magicalCoder.identifier=='mc-demo'){//表示当前操作的组件是echarts线性
            ...
            var provinceList = api.getRightPanelItemListFromConstant({key:"mc-demo",query:{attrName:"mc-attr-unknown-province"}});
            for(var i=0;i<provinceList.length;i++){
                provinceList[i].options=provinceOptions;
            }
            var cityList = api.getRightPanelItemListFromConstant({key:"mc-demo",query:{attrName:"mc-attr-unknown-city"}});
            for(var i=0;i<cityList.length;i++){
                cityList[i].options=cityOptions;
            }
            var areaList = api.getRightPanelItemListFromConstant({key:"mc-demo",query:{attrName:"mc-attr-unknown-area"}});
...
MagicalCallback.prototype.after_change_attr_callback = function (obj) {
    ...
    //test
    //示例三：三级联动示例
    if(obj.node.magicalCoder.identifier=='mc-demo'){//当前修改的是mc-demo的属性
    	var api = new MagicalApi();
		//先获取节点上配置的省市区的默认值
	    var provinceValue = obj.node.attributes['mc-attr-unknown-province'];
    	//这个数据 自己考虑是全局还是再获取一下 或者存局部变量
		var treeData = [
	        {name:"安徽省",key:"ah",children:[{name:"合肥市",key:"hf",children:[{name:"蜀山区",key:"ss"},{name:"高新区",key:"gx"}]}]},
	        {name:"浙江省",key:"zj",children:[{name:"杭州市",key:"hz",children:[{name:"滨江区",key:"bj"},{name:"西湖区",key:"xh"}]}]},
    	]
    	if(obj.name=='mc-attr-unknown-province'){//修改省属性
    		var cityOptions = [];
    		for(var i=0;i<treeData.length;i++){
	    		var provinceItem = treeData[i];
	    		if(obj.value==provinceItem.key){
	    			for(var j=0;j<provinceItem.children.length;j++){
	    				var cityItem = provinceItem.children[j];
	    				var cityMap = {};
	    				cityMap[cityItem.key]=cityItem.name;
    	...
    	}else if(obj.name=='mc-attr-unknown-city'){//修改市属性
			var areaOptions = [];
    		for(var i=0;i<treeData.length;i++){
	    		var provinceItem = treeData[i];
	    		if(provinceItem.key == provinceValue){
	    ...
	    //此处更改完一定要刷新一下 否则不会生效
        api.refreshRightAttrPane({id:obj.node.magicalCoder.id});
